...
\bibliography{~/R/DSMcompare/vignettes/DSMcompare.bib}
...
\author{
  Thomas Naake\thanks{\email{naake@stud.uni-heidelberg.de}}
}
...
with a high spatial resolution for various reasons and applications. For 
economical reasons it is unrealistic to gain information about individual 
single points, thus, state-of-the-art computational methods are being developped
to deduce information from other data mainly by using photogrammetric 
...
non-parametrical statistics. \cite{Hoehle2009}
...
(height < 2m), transition area, tree crown (local maxima > 5 m within r=2m)
...
mod1 <- read.table(system.file("model1.txt", package = "DSMcompare"), sep = ";")
mod2 <- read.table(system.file("model2.txt", package = "DSMcompare"), sep = ";")
mod3 <- read.table(system.file("model3.txt", package = "DSMcompare"), sep = ";")
